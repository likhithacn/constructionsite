...
file_path = r"C:\Users\kiran\Documents\Construction_Data_PM_Forms_All_Projects.csv"
...
filtered_df = filtered_df[
    (filtered_df['Open Actions'] >= open_actions_range[0]) & 
    (filtered_df['Open Actions'] <= open_actions_range[1])
]

filtered_df = filtered_df[
    (filtered_df['Total Actions'] >= total_actions_range[0]) & 
    (filtered_df['Total Actions'] <= total_actions_range[1])
]

st.subheader('Line Chart - Total Actions Over Time')
line_chart = px.line(filtered_df, x='Ref', y='Total Actions', title='Total Actions Over Time', labels={'Total Actions': 'Total Actions'})
st.plotly_chart(line_chart)

st.subheader('Bar Chart - Total Actions by Type')
bar_chart = px.bar(filtered_df, x='Type', y='Total Actions', title='Total Actions by Type', labels={'Total Actions': 'Total Actions'})
...
scatter_plot = px.scatter(filtered_df, x='Open Actions', y='Total Actions', color='Status', title='Open Actions vs. Total Actions')
...
violin_plot = px.violin(filtered_df, x='Type', y='Open Actions', title='Open Actions by Type', labels={'Open Actions': 'Open Actions'})
...
area_chart = px.area(filtered_df, x='Ref', y='Cumulative Total Actions', title='Cumulative Total Actions Over Time', labels={'Cumulative Total Actions': 'Cumulative Total Actions'})
st.plotly_chart(area_chart)

st.subheader('Histogram - Distribution of Total Actions')
histogram = px.histogram(filtered_df, x='Total Actions', nbins=20, title='Distribution of Total Actions', labels={'Total Actions': 'Total Actions'})
st.plotly_chart(histogram)

st.subheader('3D Scatter Plot - Open Actions, Total Actions, and Type')
scatter_3d = px.scatter_3d(filtered_df, x='Open Actions', y='Total Actions', z='Type', color='Status', title='3D Scatter Plot - Open Actions, Total Actions, and Type')
